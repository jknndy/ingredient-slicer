...
def test_convert_fractions_to_decimals_fraction_with_multiple_slashes():
    assert (
        _utils._convert_fractions_to_decimals("3/4/5") == "0.75/5"
    )  # TODO: probably should just raise an error
...
def test_convert_fractions_to_decimals_simple_fraction_then_words_then_decimal_fraction():
    assert (
        _utils._convert_fractions_to_decimals("1/2 words words words 0.5/2")
        == "0.5 words words words 0.25"
    )
...
def test_mixed_fractions_with_decimal_fractions_with_multiple_dashes_between_with_spaces_and_words():
    assert (
        _utils._convert_fractions_to_decimals("1- -1 / 2 words words words 0.5 / 2")
        == "1- -0.5 words words words 0.25"
    )
