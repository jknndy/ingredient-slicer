...
def test_parenthesis_with_equiv_quantity_unit_1():
    parse = IngredientSlicer(
        "1 cup of chopped chicken breast (about 12 ounces)", debug=False
    )
    # parse.parse()
    ...
    assert (
        parsed["secondary_quantity"] == "1"
    )  # TODO: maybe this case should get a quantity of 1, but for now it's None
...
def test_equiv_quantity_unit_equiv_with_triplet_of_quantity_units_1():
    parse = IngredientSlicer(
        "1/2 cup of chopped onions (about 4 oz, 34 grams, 0.034 kg)"
    )
    # parse.parse()
    parsed = parse.to_json()
...
def test_equiv_quantity_unit_equiv_with_triplet_of_quantity_units_outer_unit_is_weight_1():
    parse = IngredientSlicer(
        "1/2 lb of chopped onions (about 4 oz, 34 grams, 0.034 kg)"
    )
    # parse.parse()
    parsed = parse.to_json()
...
def test_quantity_and_unit_parenthesis_2():
    ...
    assert (
        parsed["secondary_quantity"] == None
    )  # TODO: maybe this case should get a quantity of 1, but for now it's None
