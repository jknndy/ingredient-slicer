...
def test_number_words_2():
    ...
    assert parsed["prep"] == []
    assert (
        parsed["food"] == "flour sugar"
    )  # TODO: not sure what the best way to handle 2 foods in one ingredient is...
    assert parsed["size_modifiers"] == []
...
def test_number_words_1():
    ...
    assert _regex_patterns.NUMBER_WORDS_MAP["two"][1].findall(example_number_words) == [
        "two"
    ]
...
def test_number_words_2():
    ...
    assert _regex_patterns.NUMBER_WORDS_MAP["three"][1].findall(
        example_number_words
    ) == ["three"]
...
def test_number_words_3():
    # numbers 2, 3, 4
    example_number_words = (
        "two cups of flour and three cups of sugar and four cups of salt"
    )
    assert _regex_patterns.NUMBER_WORDS_MAP["two"][0] == "2"
    assert _regex_patterns.NUMBER_WORDS_MAP["two"][1].findall(example_number_words) == [
        "two"
    ...
    assert _regex_patterns.NUMBER_WORDS_MAP["four"][1].findall(
        example_number_words
    ) == ["four"]
...
def test_number_words_4():
    ...
    assert _regex_patterns.NUMBER_WORDS_MAP["five"][1].findall(
        example_number_words
    ) == ["five"]
...
def test_number_words_5():
    ...
    assert _regex_patterns.NUMBER_WORDS_MAP["twenty"][1].findall(
        example_number_words
    ) == ["twenty"]
    assert _regex_patterns.NUMBER_WORDS_MAP["thirty"][0] == "30"
    assert _regex_patterns.NUMBER_WORDS_MAP["thirty"][1].findall(
        example_number_words
    ) == ["thirty"]
    assert _regex_patterns.NUMBER_WORDS_MAP["forty"][0] == "40"
    assert _regex_patterns.NUMBER_WORDS_MAP["forty"][1].findall(
        example_number_words
    ) == ["forty"]
    assert _regex_patterns.NUMBER_WORDS_MAP["fifty"][0] == "50"
    assert _regex_patterns.NUMBER_WORDS_MAP["fifty"][1].findall(
        example_number_words
    ) == ["fifty"]
