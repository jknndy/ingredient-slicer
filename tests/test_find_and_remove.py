...
def test_find_and_remove_units_pattern():
    ...
    assert _utils._find_and_remove("1 1/2 cups of flour", pattern) == "1 1/2  of flour"
    assert (
        _utils._find_and_remove("1 1/2 cups of tablespoons flour", pattern)
        == "1 1/2  of  flour"
    )
...
def test_find_and_remove_prep_words_pattern():
    ...
    assert (
        _utils._find_and_remove("firmly packed brown sugar", pattern) == "brown sugar"
    )
...
def test_find_and_remove_words_ending_in_ly_pattern():
    ...
    assert (
        _utils._find_and_remove("diced and grated cheese", pattern)
        == "diced and grated cheese"
    )
    assert (
        _utils._find_and_remove("firmly packed brown sugar", pattern)
        == "packed brown sugar"
    )
