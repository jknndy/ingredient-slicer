...
def test_split_by_parenthesis_two_sets_both_nested_depth1_1():
    ingredient = (
        "a cup of ((flour (but not too much))) and ((sugar (but not too much)))"
    )
    updated_ingredient, parenthesis_content = _utils._split_by_parenthesis(ingredient)
    assert updated_ingredient == "a cup of and"
...
def test_is_valid_parenthesis_9():
    assert (
        _utils._is_valid_parenthesis(
            "I love test (cases) even if they take (lots of (time)) to write"
        )
        == True
    )
...
def test_is_valid_parenthesis_12():
    assert (
        _utils._is_valid_parenthesis(
            "I love test (cases) even if they take lots of (time (2 (do)) to write) properly"
        )
        == True
    )
