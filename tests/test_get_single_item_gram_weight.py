...
def test_get_single_item_gram_weight_food_not_in_lookup_table():
    assert _utils._get_single_item_gram_weight("olive oil", "1") is None
...
def test_two_eggplant():
    assert _utils._get_single_item_gram_weight("eggplant", "2") == "907.0"


def test_one_eggplants():
    assert _utils._get_single_item_gram_weight("eggplants", "1") == "453.5"


def test_multiple_repeated_matching_words():
    assert _utils._get_single_item_gram_weight("eggplants eggplants", "1") is None


def test_strict_threshold_for_word_eggplant():
    assert _utils._get_single_item_gram_weight("eggplant", "1") == "453.5"
