...
class IngredientSlicer:
    ...
    def __init__(self, ingredient: str, debug=False):
        ...
        self._staged_ingredient = (
            None  # standardized ingredient but keeping the parenthesis content
        )

        self._parenthesis_content = (
            None  # content of the parenthesis removed from the ingredient string
        )
        ...
        for key, pattern in _regex_patterns.PCT_REGEX_MAP.items():
            ...
            for match in pattern_iter:
                ...
                self._standardized_ingredient = (
                    self._standardized_ingredient[:modified_start]
                    + str(replacement_str)
                    + self._standardized_ingredient[modified_end:]
                )
    ...
    def _replace_number_followed_by_inch_symbol(self) -> None:
        ...
        self._standardized_ingredient = _utils._replace_number_followed_by_inch_symbol(
            self._standardized_ingredient
        )
    ...
    def _find_and_replace_fraction_words(self) -> None:
        ...
        for key, pattern in _regex_patterns.NUMBER_WITH_FRACTION_WORD_MAP.items():
            ...
            for match in pattern_iter:
                ...
                fraction_value, decimal = _constants.FRACTION_WORDS[
                    fraction_word.lower()
                ]
    ...
    def _find_and_replace_numbers_separated_by_add_numbers(self) -> None:
        ...
        """
        add_pattern_iter = (
            _regex_patterns.NUMBERS_SEPARATED_BY_ADD_SYMBOLS_GROUPS.finditer(
                self._standardized_ingredient
            )
        )
        ...
        for match in add_pattern_iter:
            ...
            updated_value = (
                f" {_utils._make_int_or_float_str(str(first_number + second_number))} "
            )
    ...
    def _find_and_replace_casual_quantities(self):
        ...
        self._standardized_ingredient = _utils._find_and_replace_casual_quantities(
            self._standardized_ingredient
        )
    ...
    def _drop_special_dashes(self) -> None:
        ...
        self._standardized_ingredient = (
            self._standardized_ingredient.replace("—", "-")
            .replace("–", "-")
            .replace("~", "-")
        )
        return
    ...
    def _find_and_replace_prefixed_number_words(self) -> None:
        ...
        self._standardized_ingredient = _utils._find_and_replace_prefixed_number_words(
            self._standardized_ingredient
        )
    ...
    def _find_and_replace_prefixed_number_words2(self) -> None:
        ...
        """
        number_words_iter = _regex_patterns.PREFIXED_NUMBER_WORDS_GROUPS.finditer(
            self._standardized_ingredient
        )
    ...
    def _find_and_replace_number_words(self) -> None:
        ...
        """
        self._standardized_ingredient = _utils._find_and_replace_number_words(
            self._standardized_ingredient
        )
    ...
    def _clean_hyphen_padded_substrings(self) -> None:
        ...
        for substring in substrings_to_fix:
            self._standardized_ingredient = (
                _utils._find_and_remove_hyphens_around_substring(
                    self._standardized_ingredient, substring
                )
            )

    def _clean_html_and_unicode(self) -> None:
        """Unescape fractions from HTML code coded fractions to unicode fractions."""
        self._standardized_ingredient = _utils._clean_html_and_unicode(
            self._standardized_ingredient
        )
    ...
    def _replace_unicode_fraction_slashes(self) -> None:
        ...
        self._standardized_ingredient = self._standardized_ingredient.replace(
            "\u2044", "/"
        )  # could use .replace('\u2044', '\u002F'), or just .replace("⁄", "/")
    ...
    def _add_whitespace(self):
        ...
        self._standardized_ingredient = re.sub(
            pattern, r"\1 \2\3 \4", self._standardized_ingredient
        )
    ...
    def _convert_fractions_to_decimals(self) -> None:
        ...
        """
        self._standardized_ingredient = _utils._convert_fractions_to_decimals(
            self._standardized_ingredient
        )
    ...
    def _force_ws_between_numbers_and_chars(self):
        ...
        self._standardized_ingredient = _utils._force_ws_between_numbers_and_chars(
            self._standardized_ingredient
        )
    ...
    def _extract_dimensions(self) -> None:
        ...
        self._standardized_ingredient, self._dimensions = _utils._extract_dimensions(
            self._standardized_ingredient
        )
        return
    ...
    def _merge_misleading_ranges(self) -> None:
        ...
        range_iter = _regex_patterns.QUANTITY_DASH_QUANTITY_GROUPS.finditer(
            self._standardized_ingredient
        )
        ...
        for match in range_iter:
            ...
            if second_number < first_number:
                ...
                updated_value = (
                    f" {_utils._make_int_or_float_str(str(first_number + second_number))} "
                    if second_number_is_fraction
                    else f" {_utils._make_int_or_float_str(str(first_number * second_number))} "
                )
    ...
    def _fix_ranges(self):
        ...
        self._standardized_ingredient = _utils._update_ranges(
            self._standardized_ingredient, _regex_patterns.QUANTITY_DASH_QUANTITY
        )
        ...
        self._standardized_ingredient = _utils._update_ranges(
            self._standardized_ingredient, _regex_patterns.BETWEEN_QUANTITY_AND_QUANTITY
        )
        ...
        self._standardized_ingredient = _utils._update_ranges(
            self._standardized_ingredient, _regex_patterns.QUANTITY_TO_QUANTITY
        )
        ...
        self._standardized_ingredient = _utils._update_ranges(
            self._standardized_ingredient, _regex_patterns.QUANTITY_OR_QUANTITY
        )
        ...
        self._standardized_ingredient = _utils._merge_misleading_ranges(
            self._standardized_ingredient
        )
    ...
    def _remove_repeat_units(self) -> None:
        ...
        repeat_unit_matches = _regex_patterns.REPEAT_UNIT_RANGES.finditer(
            self._standardized_ingredient
        )
        ...
        for match in repeat_unit_matches:
            ...
            if unit1 == unit2:
                self._standardized_ingredient = self._standardized_ingredient.replace(
                    original_string, f"{quantity1} - {quantity2} {unit1}"
                )

    def _remove_repeat_units_in_ranges(self) -> None:

        self._standardized_ingredient = _utils._remove_repeat_units_in_ranges(
            self._standardized_ingredient
        )
    ...
    def _remove_x_separators(self):
        ...
        self._standardized_ingredient = _regex_patterns.X_AFTER_NUMBER.sub(
            replace_x, self._standardized_ingredient
        )
    ...
    def _merge_spaced_numbers(self, spaced_numbers: str) -> str:
        ...
        try:
            merged_totals = [
                _utils._make_int_or_float_str(
                    str(float(first) + float(second))
                    if float(second) < 1
                    else str(float(first) * float(second))
                )
                for first, second in split_numbers
            ]
        except:
    ...
    def _which_merge_on_spaced_numbers(self, spaced_numbers: str) -> str:
        ...
        try:
            add_or_multiply = [
                "add" if float(second) < 1 else "multiply"
                for first, second in split_numbers
            ]
        except:
            warnings.warn(
                f"error while deciding whether to note '{split_numbers}' as numbers to 'add' or 'multiply'..."
            )
            add_or_multiply = [""]
    ...
    def _merge_multi_nums(self) -> None:
        ...
        spaced_matches = re.finditer(
            _regex_patterns.SPACE_SEP_NUMBERS, self._standardized_ingredient
        )
        ...
        for match in spaced_matches:
            ...
            unit_after_match = re.search(
                _regex_patterns.UNITS_PATTERN, self._standardized_ingredient[end:]
            )
    ...
    def _replace_a_or_an_quantities(self) -> None:
        ...
        """
        self._standardized_ingredient = _utils._replace_a_or_an_quantities(
            self._standardized_ingredient
        )
        return
    ...
    def _separate_dimensions(self) -> None:
        ...
        self._standardized_ingredient, self._dimensions = _utils._separate_dimensions(
            self._standardized_ingredient
        )
        return
    ...
    def _separate_parenthesis(self):
        ...
        ingredient_without_parenthesis, parenthesis = _utils._split_by_parenthesis(
            self._standardized_ingredient
        )
        ...
        self._standardized_ingredient = (
            ingredient_without_parenthesis  # TODO: TESTING THIS OUT
        )
    ...
    def _standardize(self):
        ...
        self._standardized_ingredient = _utils._remove_extra_whitespaces(
            self._standardized_ingredient
        )
    ...
    def extract_first_quantity_unit(self) -> None:
        ...
        basic_unit_matches = _regex_patterns.QUANTITY_BASIC_UNIT_GROUPS.findall(
            self._standardized_ingredient
        )  # TODO: testing
        ...
        # debugging message
        basic_units_message = (
            f"Valid basic units: {valid_basic_units}"
            if valid_basic_units
            else f"No valid basic units found..."
        )
        ...
        nonbasic_unit_matches = _regex_patterns.QUANTITY_NON_BASIC_UNIT_GROUPS.findall(
            self._standardized_ingredient
        )  # TODO: testing
        ...
        # debugging message
        nonbasic_units_message = (
            f"Valid non basic units: {valid_nonbasic_units}"
            if valid_nonbasic_units
            else f"No valid non basic units found..."
        )
        ...
        quantity_matches = _regex_patterns.ALL_NUMBERS.findall(
            self._standardized_ingredient
        )  # TODO: testing
        unit_matches = _regex_patterns.UNITS_PATTERN.findall(
            self._standardized_ingredient
        )  # TODO: testing
        ...
        # debugging messages
        all_quantities_message = (
            f"Valid quantities: {valid_quantities}"
            if valid_quantities
            else f"No valid quantities found..."
        )
        all_units_message = (
            f"Valid units: {valid_units}" if valid_units else f"No valid units found..."
        )
    ...
    def _check_if_required_parenthesis(self, parenthesis_list: list) -> bool:
        ...
        optional_match_flag = any(
            [
                True if _regex_patterns.OPTIONAL_STRING.findall(i) else False
                for i in parenthesis_list
            ]
        )
        required_match_flag = any(
            [
                True if _regex_patterns.REQUIRED_STRING.findall(i) else False
                for i in parenthesis_list
            ]
        )

        # check if any of the words in the parenthesis content are in the optional or required sets
        optional_str_flag = any(
            [
                any(
                    [
                        True if word in optional_set else False
        ...
        required_str_flag = any(
            [
                any(
                    [
                        True if word in required_set else False
                        for word in i.replace("(", "")
                        .replace(")", "")
                        .replace(",", " ")
                        .split()
        ...
        is_required = (True if required_match_flag or required_str_flag else False) or (
            False if optional_match_flag or optional_str_flag else True
        )
    ...
    def _check_if_required_string(self, ingredient: str) -> bool:
        ...
        optional_match_flag = (
            True if _regex_patterns.OPTIONAL_STRING.findall(ingredient) else False
        )
        required_match_flag = (
            True if _regex_patterns.REQUIRED_STRING.findall(ingredient) else False
        )
    ...
    def _is_ingredient_required(self) -> bool:
        ...
        ingredient_is_required = self._check_if_required_string(
            self._standardized_ingredient
        )  # TODO: testing this out

        # check the parenthesis content for the word "optional" or "required"
        parenthesis_is_required = self._check_if_required_parenthesis(
            self._parenthesis_content
        )
    ...
    def _address_quantity_only_parenthesis(self, parenthesis: str) -> None:
        ...
        numbers_only = _utils._extract_quantities_only(
            parenthesis
        ...
        is_approximate_quantity = _utils._is_approximate_quantity_only_parenthesis(
            parenthesis
        )
    ...
    def _address_equivalence_parenthesis(self, parenthesis: str) -> None:
        ...
        equivalent_quantity_unit = _utils._extract_equivalent_quantity_units(
            parenthesis
        )
        ...
        if not equivalent_quantity_unit or any(
            [
                True if i in _constants.QUANTITY_PER_UNIT_STRINGS else False
                for i in split_parenthesis
            ]
        ):
        ...
        parenthesis_suffix, parenthesis_quantity, parenthesis_unit = (
            equivalent_quantity_unit[0]
        )
        ...
        if self._quantity and self._unit:
            ...
            if parenthesis_unit_is_basic and unit_is_basic:
                description = (
                    f"maybe quantity/unit is: {parenthesis_quantity}/{parenthesis_unit}"
                )
                self._parenthesis_notes.append(description)
        ...
        description = (
            f"used quantity/units from parenthesis with equivalent quantity/units"
        )
        self._parenthesis_notes.append(description)
    ...
    def _address_quantity_unit_only_parenthesis(self, parenthesis: str) -> None:
        ...
        if self._quantity and self._unit:
            ...
            if parenthesis_unit_is_basic and not unit_is_basic:
                ...
                updated_quantity = str(
                    float(self._quantity) * float(parenthesis_quantity)
                )
    ...
    def _add_standard_units(self) -> None:
        ...
        if (
            ...
            and self._secondary_unit in _constants.UNIT_TO_STANDARD_UNIT
        ):
            self._standardized_secondary_unit = _constants.UNIT_TO_STANDARD_UNIT[
                self._secondary_unit
            ]
    ...
    def _prioritize_weight_units(self) -> None:
        ...
        if (
            self._unit not in _constants.WEIGHT_UNITS_SET
            and self._secondary_unit in _constants.WEIGHT_UNITS_SET
        ):
            ...
            self._quantity, self._secondary_quantity = (
                self._secondary_quantity,
                self._quantity,
            )
            self._unit, self._secondary_unit = self._secondary_unit, self._unit
            self._standardized_unit, self._standardized_secondary_unit = (
                self._standardized_secondary_unit,
                self._standardized_unit,
            )
    ...
    def _find_and_remove(self, string: str, pattern: re.Pattern) -> str:
        ...
        for match in pattern_iter:
            ...
            # Construct the modified string with the replacement applied
            string = (
                string[:modified_start] + str(replacement_str) + string[modified_end:]
            )
    ...
    def _extract_prep_words(self, ingredient: str) -> str:
        ...
        ly_ending_words = list(
            ly_ending_words - _constants.APPROXIMATE_STRINGS
        )  # remove any accidental match overlaps like "approximately"

        prep_words = _regex_patterns.PREP_WORDS_PATTERN.findall(ingredient)
        prep_words.extend(
            ly_ending_words
        )  # combine the prep words with the 'ly' ending words
    ...
    def _add_gram_weights(self):
        ...
        # ingredient = "2 1/2 cups of sugar (about 1/2 inch squares of sugar)"
        grams_map = _utils._get_gram_weight(
            self._food, self._quantity, self._unit, "dice"
        )
    ...
    def _parse(self):
        ...
        (
            print(f"Is the ingredient required? {self._is_required}")
            if self.debug
            else None
        )
        ...
        (
            print(
                f"Adding standard unit names for {self._unit} and {self._secondary_unit}"
            )
            if self.debug
            else None
        )
        ...
        # print(f"Extracting food words (version {self.extract_version})") if self.debug else None
        self._prep = self._extract_prep_words(
            self._staged_ingredient
        )  # TODO: testing using staged_ingredient
        self._size_modifiers = self._extract_size_modifiers(
            self._staged_ingredient
        )  # TODO: testing using staged_ingredient
